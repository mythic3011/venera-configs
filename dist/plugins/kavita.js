class Kavita extends ComicSource {
    constructor(...e) {
        ...
        this.minAppVersion = "1.4.0", this.url = resolvePluginUpdateUrl("kavita.js"), this.settings = {
            ...
            login: async (e, t) => {
                ...
                const r = await Network.post(this.buildUrl(KAVITA_ROUTES.loginPath()), this.headers, {
                    username: e,
                    password: t
                });
                ...
                if (200 === r.status) {
                    const t = parseSelfHostedJsonBody(r.body) || {};
            ...
            load: async (e, t, r, o) => {
                ...
                if (r && r.length) {
                    const [e, t] = r[0].split(",");
                    a.sortOptions.sortField = parseInt(e), a.sortOptions.isAscending = "true" === t;
                }
                const i = t.split(":");
                if ("library" === i[0] && i[1]) {
                    const e = i[1];
                ...
                if ([ "all", "library", "genre", "author" ].includes(i[0])) {
            ...
            load: async (e, t, r) => {
                ...
                const {comics: a, totalPages: i} = await this.fetchSeriesList(KAVITA_ROUTES.seriesV2Path(), {
                    PageNumber: r,
                    PageSize: 30
                }, o);
                return {
            ...
            loadInfo: async e => {
                const t = await this.getJson(KAVITA_ROUTES.seriesDetailsPath(e)), r = await this.getJson(KAVITA_ROUTES.seriesMetadataPath(), {
                    seriesId: e
                }), o = (await this.getJson(KAVITA_ROUTES.seriesVolumesPath(), {
                    seriesId: e
                })).flatMap(e => e.chapters || []).reduce((e, {id: t, titleName: r, files: o}) => {
                    let a = r;
                    return a || (a = o[0].filePath.split("/").pop()), e[t] = a, e;
                }, {}), a = r.writers.map(e => e.name), i = this.loadData("apiKey"), l = {}, n = this.isReadable(t.format);
                return console.log(t), a.length && (l["作者"] = a), r.genres.length && (l["类型"] = r.genres.map(e => e.title)),
                r.tags.length && (l["标签"] = r.tags.map(e => e.title)), n || (l["提示"] = [ "该系列包含的项目暂不支持阅读" ]),
                new ComicDetails({
                    title: t.name,
                    subtitle: a.join(", "),
            ...
            loadEp: async (e, t) => {
                const r = await this.getJson(KAVITA_ROUTES.chapterPath(), {
                    chapterId: t
                }), o = r.pages;
                if (!this.isReadable(r.format)) throw "该项目暂不支持阅读";
                const a = this.loadData("apiKey"), i = 4 === r.format;
                return {
                    images: Array.from({
                        length: o
                    }, (e, r) => this.buildUrl(KAVITA_ROUTES.readerImagePath(), {
                        chapterId: t,
                        page: r,
                        apiKey: a,
                        extractPdf: i
            ...
            onClickTag: (e, t) => {
                if ("类型" === e) {
                    var r;
                    const e = null == (r = this.loadData("kavita_genres").find(e => e.title === t)) ? void 0 : r.id;
                    if (e) return {
                        action: "category",
                        keyword: t,
                        param: `genre:${e}`
    ...
    async fetchSeriesList(e, t, r) {
        const {content: o, page: a} = await this.postJson(e, t, r), i = Array.isArray(o) ? o : [], l = this.loadData("apiKey");
        return {
            comics: i.map(e => this.parseSeries(e, l)).filter(Boolean),
            totalPages: a.totalPages
        };
    }
    parseSeries(e, t) {
        if (!e) return null;
        const r = e.id || e.seriesId, o = e.name;
        return new Comic({
            id: `${r}`,
            title: o,
            cover: this.buildUrl(KAVITA_ROUTES.seriesCoverPath(), {
    ...
    async postJson(e, t, r) {
        const o = await postSelfHostedJson(this, e, t, r);
        return {
            content: o.body,
...
function createOffsetSearchLoader(e) {
    ...
    return async function(e, t) {
        ...
        const s = t && "object" == typeof t && !Array.isArray(t) ? t : {}, u = Number(s.page || 1), c = stripSelfHostedTrailingSlash(e.baseUrl), d = o(s), p = readSelfHostedOffset(e, d, u), h = buildSelfHostedQueryFromSource(a(s, p)), m = h ? `${c}${r}?${h}` : `${c}${r}`, y = await Network.get(m, e.headers);
        if (200 !== y.status) throw `${n}: ${y.status}`;
        const f = parseSelfHostedJsonBody(y.body) || {}, g = Array.isArray(f.data) ? f.data : [], C = g.map(t => i(t, {
            source: e,
            base: c,
            input: s
        })), S = g.length;
        updateSelfHostedOffset(e, d, S);
        const k = "number" == typeof f.recordsFiltered && f.recordsFiltered >= 0 ? f.recordsFiltered : p + S, L = S || 1, P = Math.max(1, Math.ceil(k / L));
        return l && l({
            source: e,
            input: s,
            start: p,
...
const refreshKavitaReferenceDataFeature = createSelfHostedReferenceCacheFeature({
    ...
    loadPayload: async e => {
        const [t, r, o] = await Promise.all([ e.getJson(KAVITA_ROUTES.librariesPath()), e.getJson(KAVITA_ROUTES.genresPath()), e.getJson(KAVITA_ROUTES.peopleByRolePath(), {
            role: 3
        }) ]);
        return {
            libraries: t,
    ...
    savePayload: (e, t) => {
        const r = Array.isArray(null == t ? void 0 : t.libraries) ? t.libraries.filter(e => e && e.id) : [];
        e.saveData("kavita_libraries", r.map(e => ({
            id: e.id,
            name: e.name
