class Ehentai extends ComicSource {
  ...
  /**
   * Ephemeral runtime-only request state. Do not persist.
   */
  requestQueues = new Map();
  inflightRequests = new Map();
  cooldownUntilByDomain = new Map();
  ...
  getGalleryCacheKey(id) {
    return id;
  }

  getThumbnailCacheKey(id, next) {
    return `${id}::${next ?? "0"}`;
  }

  getCooldownMessage(domainKey) {
    let until = this.cooldownUntilByDomain.get(domainKey) ?? 0;
    let seconds = Math.max(1, Math.ceil((until - Date.now()) / 1000));
    return `Request cooldown active for ${domainKey}. Please wait ${seconds}s before retrying.`;
  }
  ...
  async ensureRemoteTranslationLoaded() {
    if (this.remoteTranslation != null) {
      return this.remoteTranslation;
    }
    if (this.translationLoadPromise != null) {
      return await this.translationLoadPromise;
    }
    this.translationLoadPromise = (async () => {
      try {
        let res = await this.safeGet(
          this.translationUrl,
          {},
          {
            requestKey: "translation:ehentai",
            cacheTime: "long",
            stopOnAbuse: false,
            retries: 0,
            expectedStatus: [200],
  ...
  async syncEhCookiesToExhentai() {
    let cookies = await Network.getCookies("https://e-hentai.org");
    let allowed = ["ipb_member_id", "ipb_pass_hash", "igneous", "star", "sk"];
    cookies = cookies.filter((item) => allowed.includes(item.name));
    cookies.forEach((c) => {
  ...
  isTransientRequestError(error) {
    let msg = String(error ?? "").toLowerCase();
    return (
  ...
  setCooldown(domainKey, ms) {
    let until = Date.now() + ms;
    let current = this.cooldownUntilByDomain.get(domainKey) ?? 0;
    if (until > current) {
      this.cooldownUntilByDomain.set(domainKey, until);
    }
  }
  ...
  async enqueueDomainRequest(domainKey, task) {
    let previous = this.requestQueues.get(domainKey) ?? Promise.resolve();
    let current = previous.catch(() => {}).then(task);
    this.requestQueues.set(domainKey, current);
    try {
      return await current;
    } finally {
      if (this.requestQueues.get(domainKey) === current) {
        this.requestQueues.delete(domainKey);
      }
    }
  }

  async safeRequest(method, url, headers, body, options = {}) {
    let domainKey = options.domainKey ?? this.getDomainKey(url);
    let requestKey = options.requestKey ?? `${method}:${url}`;
    let inflightKey = `${domainKey}:${requestKey}`;
    if (options.dedupe !== false && this.inflightRequests.has(inflightKey)) {
      return await this.inflightRequests.get(inflightKey);
    }

    let task = this.enqueueDomainRequest(domainKey, async () => {
      let cooldownUntil = this.cooldownUntilByDomain.get(domainKey) ?? 0;
      if (cooldownUntil > Date.now()) {
        throw this.getCooldownMessage(domainKey);
      }

      let retries = options.retries ?? (method === "GET" ? 1 : 0);
      let attempt = 0;
      while (true) {
        try {
          let requestOptions = Object.assign({}, headers ?? {});
          if (options.cacheTime != null) {
            requestOptions["cache-time"] = options.cacheTime;
          }
          if (options.cookie != null) {
            requestOptions["cookie"] = options.cookie;
          }
          if (options.preventParallel != null) {
            requestOptions["prevent-parallel"] = options.preventParallel;
          }
          if (options.httpClient != null) {
            requestOptions["http_client"] = options.httpClient;
          }

          let res =
            method === "GET"
              ? await Network.get(url, requestOptions)
              : await Network.post(url, requestOptions, body);

          let expectedStatus = options.expectedStatus ?? [200];
          if (!expectedStatus.includes(res.status)) {
            if (res.status === 403 || res.status === 429) {
              this.setCooldown(domainKey, options.cooldownMs ?? 10 * 60 * 1000);
            }
            throw `Invalid status code: ${res.status}`;
          }
          if (options.stopOnAbuse && this.isAbuseSignalBody(res.body)) {
            this.setCooldown(domainKey, options.cooldownMs ?? 10 * 60 * 1000);
            throw this.getCooldownMessage(domainKey);
          }
          if (options.requireHtml && String(res.body ?? "").trim()[0] !== "<") {
            if (String(res.body ?? "").includes("IP")) {
              this.setCooldown(domainKey, options.cooldownMs ?? 10 * 60 * 1000);
          ...
          if (attempt >= retries || !this.isTransientRequestError(e)) {
            throw e;
          }
          await this.sleep(Math.min(1500 * (attempt + 1), 5000));
          attempt += 1;
        }
      }
    });
  ...
  async safeGet(url, headers = {}, options = {}) {
    return await this.safeRequest("GET", url, headers, null, options);
  }

  async safePost(url, headers = {}, body = null, options = {}) {
    return await this.safeRequest("POST", url, headers, body, options);
  }

  async ensureImageSession(comicId) {
    if (this.imageSessionCache.has(comicId)) {
      return await this.imageSessionCache.get(comicId);
    }
    let task = (async () => {
      let first = await this.comic.loadThumbnails(comicId);
      let key = await this.comic.getKey(first.urls[0]);
      return {
        firstThumbnails: first,
        key: key,
        firstPageSize: first.thumbnails.length,
      };
    })();
    this.imageSessionCache.set(comicId, task);
    try {
      return await task;
    } catch (e) {
      this.imageSessionCache.delete(comicId);
      throw e;
    }
  }

  async checkEHEvent() {
    this.ensureRemoteTranslationLoaded().catch(() => {});
    if (!this.isLogged) {
      return;
    }
  ...
  account = {
    ...
    loginWithWebview: {
      ...
      onLoginSuccess: async () => {
        await this.syncEhCookiesToExhentai();
      },
    },
    ...
    loginWithCookies: {
      ...
      validate: async (values) => {
        ...
        for (let i = 0; i < values.length; i++) {
          let cookieName = this.account.loginWithCookies.fields[i];
          if (!authCookieNames.includes(cookieName)) {
            continue;
          }
          cookies.push(
            new Cookie({
        ...
        let res = await this.safeGet(
          ...
          {
            requestKey: "auth:validate-forum-session",
            stopOnAbuse: false,
            retries: 0,
          },
        );
    ...
    logout: () => {
      ...
      this.cooldownUntilByDomain.clear();
    },
  ...
  async onLoadFailed() {
    await this.syncEhCookiesToExhentai();
    this.setCooldown(this.getDomainKey(this.baseUrl), 10 * 60 * 1000);
  ...
  favorites = {
    ...
    addOrDelFavorite: async (comicId, folderId, isAdding, favoriteId) => {
      ...
      let url = `${this.baseUrl}/gallerypopups.php?gid=${id}&t=${token}&act=addfav`;
      if (isAdding) {
        let res = await this.safePost(
          url,
          {
        ...
        let res = await this.safePost(
          ...
          {
            requestKey: `favorite:delete:${comicId}`,
            stopOnAbuse: true,
            requireHtml: true,
            retries: 0,
          },
  ...
  comic = {
     ...
     */
    loadInfo: async (id) => {
      let cacheKey = this.getGalleryCacheKey(id);
      if (this.galleryInfoCache.has(cacheKey)) {
        return this.galleryInfoCache.get(cacheKey);
      }
      try {
        this.checkEHEvent();
      } catch (_) {}
      ...
      this.galleryInfoCache.set(cacheKey, comic);
      return comic;
    },
    /**
    ...
    loadThumbnails: async (id, next) => {
      let cacheKey = this.getThumbnailCacheKey(id, next);
    ...
    onImageLoad: async (image, comicId, epId, nl) => {
      let session = await this.ensureImageSession(comicId);
      let first = session.firstThumbnails;
      let key = session.key;
      let page = Number(image);
      let retryDepth =
        nl && String(nl).includes("::retry=")
          ? Number(String(nl).split("::retry=").at(1) ?? "0")
          : 0;

      let parseRetryNl = (value) => {
        if (!value) {
          return null;
        }
        return String(value).split("::retry=")[0];
      };
      ...
      let getImageFromApi = async (nl) => {
        if (key.mpvkey) {
          let res = await this.safePost(
            ...
            {
              requestKey: `image:dispatch:${comicId}:${page}`,
              retries: 0,
              stopOnAbuse: true,
            },
          );
          ...
          if (page < first.thumbnails.length) {
            ...
            let onePageLength = session.firstPageSize;
          ...
          let res = await this.safePost(
            ...
            {
              requestKey: `image:showpage:${comicId}:${page}`,
              retries: 0,
              stopOnAbuse: true,
            },
          );
      ...
      if (res.nl && retryDepth < 1) {
        onLoadFailed = async () => {
          return this.comic.onImageLoad(
            image,
            comicId,
            epId,
            `${res.nl}::retry=${retryDepth + 1}`,
          );
        };
      }
    ...
    sendComment: async (comicId, subId, content, replyTo) => {
      let res = await this.safePost(
        ...
        {
          requestKey: `comment:send:${comicId}`,
          stopOnAbuse: true,
          requireHtml: true,
          retries: 0,
          expectedStatus: [200, 201, 202, 204, 302],
        },
      );
      if (res.status >= 400) {
        throw `Invalid status code: ${res.status}`;
    ...
    voteComment: async (id, subId, commentId, isUp, isCancel) => {
      ...
      let res = await this.safePost(
        ...
        {
          requestKey: `comment:vote:${id}:${commentId}:${isUp ? "up" : "down"}`,
          stopOnAbuse: true,
          retries: 0,
        },
      );
    ...
    archive: {
      ...
      getDownloadUrl: async (cid, aid) => {
        ...
        if (aid.startsWith("h@h_")) {
          ...
          let hathRes = await this.safePost(
            `${this.baseUrl}/archiver.php?gid=${gid}&token=${token}`,
            {
              "Content-Type": "application/x-www-form-urlencoded",
            },
        ...
        let res = await this.safePost(
          ...
          data,
          {
            requestKey: `archive:download:${cid}:${aid}`,
            stopOnAbuse: true,
            requireHtml: true,
            retries: 0,
          },
        ...
        let res2 = await this.safeGet(
          link,
          {},
          {
            requestKey: `archive:redirect:${cid}:${aid}`,
            httpClient: "dart:io",
            stopOnAbuse: false,
            requireHtml: true,
          },
  ...
  translate(key) {
    ...
    return (
      this.remoteTranslation?.[locale]?.[key] ??
      this.translation?.[locale]?.[key] ??
      key
    );
  }
}
