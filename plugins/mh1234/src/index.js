const MH1234_IMAGE_BASE_URL = normalizeWebSourceBaseUrl(
  "https://gmh1234.wszwhg.net",
);
const MH1234_ROUTE_PATHS = {
  LIST: "/list/",
  SEARCH: "/search/",
  COMIC: "/comic",
};

function normalizeMh1234EpisodeImagePath(path) {
  return String(path || "")
    .replaceAll("\\", "")
    .replace(/^\/+/, "");
}

function buildMh1234ComicPath(id) {
  return joinWebSourcePath(MH1234_ROUTE_PATHS.COMIC, [`${id}.html`]);
}

function buildMh1234EpisodePath(seriesId, chapterId) {
  return joinWebSourcePath(MH1234_ROUTE_PATHS.COMIC, [
    seriesId,
    `${chapterId}.html`,
  ]);
}

function buildMh1234FilterQuery(params, options, page) {
  const safeOptions = Array.isArray(options) ? options : [];
  return {
    filter: `${params}-${safeOptions[0]}-${safeOptions[1]}-${safeOptions[2]}`,
    sort: safeOptions[3],
    page,
  };
}
...
class MH1234 extends ComicSource {
  ...
  // update url
  url = resolvePluginUpdateUrl("mh1234.js");
    ...
    for (let comic of doc.querySelectorAll(".itemBox")) {
      comics.push(
        new Comic({
          ...
          cover: toWebSourceAbsoluteUrl(
            comic.querySelector("img").attributes["src"],
            this.baseUrl,
          ),
        }),
      );
  ...
  categoryComics = {
    load: async (category, params, options, page) => {
      if (params.endsWith(".html")) {
        const res = await Network.get(buildWebSourceUrl(this.baseUrl, params));
        if (res.status !== 200) {
          throw `Invalid status code: ${res.status}`;
        }
        return this.parseComics(res.body, true);
      } else {
        const requestUrl = buildWebSourceUrl(
          this.baseUrl,
          MH1234_ROUTE_PATHS.LIST,
          buildMh1234FilterQuery(params, options, page),
        );
        const res = await Network.get(requestUrl);
  ...
  search = {
    load: async (keyword, options, page) => {
      const requestUrl = buildWebSourceUrl(this.baseUrl, MH1234_ROUTE_PATHS.SEARCH, {
        keywords: keyword,
        sort: Array.isArray(options) ? options[0] : undefined,
        page,
      });
      const res = await Network.get(requestUrl);
      if (res.status !== 200) {
  ...
  comic = {
    loadInfo: async (id) => {
      const res = await Network.get(
        buildWebSourceUrl(this.baseUrl, buildMh1234ComicPath(id)),
      );
      if (res.status !== 200) {
        throw `Invalid status code: ${res.status}`;
      }
      ...
      const cover = toWebSourceAbsoluteUrl(
        doc.querySelector(".pic").querySelector("img").attributes["src"],
        this.baseUrl,
      );
    ...
    loadEp: async (comicId, epId) => {
      ...
      const res = await Network.get(
        buildWebSourceUrl(this.baseUrl, buildMh1234EpisodePath(ids[0], ids[1])),
      );
      if (res.status !== 200) {
        throw `Invalid status code: ${res.status}`;
      }
      ...
      for (let i = 0; i < images.length; i++) {
        images[i] = buildWebSourceUrl(
          MH1234_IMAGE_BASE_URL,
          joinWebSourcePath("/", [cpath, normalizeMh1234EpisodeImagePath(images[i])]),
        );
      }
      return { images };
    },
