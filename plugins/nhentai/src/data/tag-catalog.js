...
const NHENTAI_TAG_VALUES = Object.freeze(Object.values(NHENTAI_TAG_CATALOG));

function normalizeNhentaiTagIds(tagIdsOrText) {
  if (Array.isArray(tagIdsOrText)) {
    return tagIdsOrText.map((value) => String(value));
  }
  return String(tagIdsOrText || "")
    .split(/\s+/)
    .map((value) => value.trim())
    .filter(Boolean);
}

function collectNhentaiTagNames(tagIdsOrText) {
  return normalizeNhentaiTagIds(tagIdsOrText)
    .map((tagId) => NHENTAI_TAG_CATALOG[tagId])
