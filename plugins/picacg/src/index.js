class Picacg extends ComicSource {
  ...
  url = resolvePluginUpdateUrl("picacg.js");
  ...
  get routes() {
    if (!this._routes) {
      this._routes = createPicacgRouteHelpers();
    }
    return this._routes;
  }

  buildRequestUrl(path) {
    return buildPicacgEndpointUrl(this.loadSetting("base_url"), path);
  }

  buildRequestHeaders(method, request, token) {
    return this.buildHeaders(method, request.signaturePath, token);
  }
  ...
  account = {
    ...
    login: async (account, pwd) => {
      const request = this.routes.authSignInRequest();
      let res = await Network.post(
        this.buildRequestUrl(request.path),
        this.buildRequestHeaders("POST", request),
        {
          email: account,
          password: pwd,
        },
  ...
  explore = [
    {
      ...
      load: async (page) => {
        ...
        const request = this.routes.randomComicsRequest();
        let res = await Network.get(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("GET", request, this.loadData("token")),
        );
        if (res.status === 401) {
    ...
    },
    {
      ...
      load: async (page) => {
        ...
        const request = this.routes.latestComicsRequest({
          page,
          sort: "dd",
        });
        let res = await Network.get(
    ...
    },
    {
      ...
      load: async (page) => {
        ...
        const request = this.routes.leaderboardRequest({ option: "H24" });
        let res = await Network.get(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("GET", request, this.loadData("token")),
        );
    ...
    },
    {
      ...
      load: async (page) => {
        ...
        const request = this.routes.leaderboardRequest({ option: "D7" });
        let res = await Network.get(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("GET", request, this.loadData("token")),
        );
    ...
    },
    {
      ...
      load: async (page) => {
        ...
        const request = this.routes.leaderboardRequest({ option: "D30" });
        let res = await Network.get(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("GET", request, this.loadData("token")),
        );
  ...
  categoryComics = {
    load: async (category, param, options, page) => {
      let type = param ?? "c";
      const request = this.routes.categoryComicsRequest({
        page,
        type,
    ...
    ranking: {
      ...
      load: async (option, page) => {
        const request = this.routes.leaderboardRequest({ option });
        let res = await Network.get(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("GET", request, this.loadData("token")),
        );
  ...
  search = {
    load: async (keyword, options, page) => {
      const request = this.routes.advancedSearchRequest({ page });
      let res = await Network.post(
        this.buildRequestUrl(request.path),
        this.buildRequestHeaders("POST", request, this.loadData("token")),
        JSON.stringify({
          keyword: keyword,
          sort: options[0],
        }),
  ...
  favorites = {
    ...
    addOrDelFavorite: async (comicId, folderId, isAdding) => {
      const request = this.routes.comicFavoriteRequest({ comicId });
      let res = await Network.post(
        this.buildRequestUrl(request.path),
        this.buildRequestHeaders("POST", request, this.loadData("token")),
        "{}",
    ...
    loadComics: async (page, folder) => {
      ...
      const request = this.routes.userFavoritesRequest({ page, sort });
      let res = await Network.get(
        this.buildRequestUrl(request.path),
        this.buildRequestHeaders("GET", request, this.loadData("token")),
      );
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      let infoLoader = async () => {
        const request = this.routes.comicInfoRequest({ comicId: id });
        let res = await Network.get(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("GET", request, this.loadData("token")),
        );
      ...
      let epsLoader = async () => {
        ...
        let allEps = [];
        while (true) {
          const request = this.routes.comicEpsRequest({
            comicId: id,
            page: i,
          });
          let res = await Network.get(
      ...
      let relatedLoader = async () => {
        const request = this.routes.comicRecommendationRequest({ comicId: id });
        let res = await Network.get(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("GET", request, this.loadData("token")),
        );
    ...
    loadEp: async (comicId, epId) => {
      ...
      let i = 1;
      while (true) {
        const request = this.routes.comicEpPagesRequest({
          comicId,
          epId,
          page: i,
        });
    ...
    likeComic: async (id, isLike) => {
      const request = this.routes.comicLikeRequest({ comicId: id });
    ...
    loadComments: async (comicId, subId, page, replyTo) => {
      ...
      if (replyTo) {
        const request = this.routes.commentChildrenRequest({ replyTo, page });
        let res = await Network.get(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("GET", request, this.loadData("token")),
        );
        ...
        const request = this.routes.comicCommentsRequest({ comicId, page });
        let res = await Network.get(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("GET", request, this.loadData("token")),
        );
    ...
    sendComment: async (comicId, subId, content, replyTo) => {
      if (replyTo) {
        const request = this.routes.commentReplyRequest({ replyTo });
        let res = await Network.post(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("POST", request, this.loadData("token")),
          JSON.stringify({
        ...
        const request = this.routes.comicCommentRequest({ comicId });
        let res = await Network.post(
          this.buildRequestUrl(request.path),
          this.buildRequestHeaders("POST", request, this.loadData("token")),
          JSON.stringify({
    ...
    likeComment: async (comicId, subId, commentId, isLike) => {
      const request = this.routes.commentLikeRequest({ commentId });
      let res = await Network.post(
        this.buildRequestUrl(request.path),
        this.buildRequestHeaders("POST", request, this.loadData("token")),
        "{}",
    ...
    onClickTag: (namespace, tag) => {
      return resolvePicacgTagAction(namespace, tag);
    },
  };
