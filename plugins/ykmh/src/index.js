...
const YKMH_DESKTOP_BASE_URL = normalizeWebSourceBaseUrl("https://www.ykmh.net");
const YKMH_MOBILE_BASE_URL = normalizeWebSourceBaseUrl("https://m.ykmh.net");
const YKMH_DESKTOP_DEFAULT_COVER = buildWebSourceUrl(
  YKMH_DESKTOP_BASE_URL,
  "/images/default/cover.png",
);
const YKMH_MOBILE_DEFAULT_COVER = buildWebSourceUrl(
  YKMH_MOBILE_BASE_URL,
  "/images/default/cover.png",
);

function toYkmhDesktopUrl(pathOrUrl) {
  return toWebSourceAbsoluteUrl(pathOrUrl, YKMH_DESKTOP_BASE_URL);
}

function toYkmhMobileUrl(pathOrUrl) {
  return toWebSourceAbsoluteUrl(pathOrUrl, YKMH_MOBILE_BASE_URL);
}

function toYkmhMobileComicUrl(pathOrUrl) {
  const rewritten = replaceWebSourceBaseUrl(
    String(pathOrUrl || "").trim(),
    YKMH_DESKTOP_BASE_URL,
    YKMH_MOBILE_BASE_URL,
  );
  return ensureWebSourceTrailingSlash(toYkmhMobileUrl(rewritten));
}

function buildYkmhCategoryRequestUrl(param, sort, page) {
  if (param === "" || param === undefined) {
    return buildWebSourceUrl(YKMH_DESKTOP_BASE_URL, `/list/${sort}/`, { page });
  }
  return buildWebSourceUrl(
    YKMH_DESKTOP_BASE_URL,
    `/list/${param}/${sort}/${page}/`,
  );
}

function buildYkmhSearchRequestUrl(keyword, page) {
  const query = { keywords: keyword };
  if (page && page > 1) {
    query.page = page;
  }
  return buildWebSourceUrl(YKMH_DESKTOP_BASE_URL, "/search/", query);
}
...
class YKMHSource extends ComicSource {
  ...
  version = "1.0.0";
  minAppVersion = "1.4.0";
  url = resolvePluginUpdateUrl("ykmh.js");
      ...
      load: async (page) => {
        ...
        function parseHotCarousel(html) {
          ...
          while ((match = carouselPattern.exec(html)) !== null) {
            let cover = toYkmhDesktopUrl(match[2]);
        ...
        function parseLatestComics(html) {
          ...
          while ((match = comicPattern.exec(html)) !== null) {
            let cover = toYkmhDesktopUrl(match[4]);
  ...
  categoryComics = {
    load: async (category, param, options, page) => {
      ...
      let url = buildYkmhCategoryRequestUrl(param, sort, page);
      ...
      function parseComicsList(html) {
        ...
        let match;
        while ((match = comicPattern.exec(html)) !== null) {
          let cover = toYkmhDesktopUrl(match[3]);
  ...
  search = {
    load: async (keyword, options, page) => {
      let url = buildYkmhSearchRequestUrl(keyword, page);
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      let targetUrl = toYkmhMobileComicUrl(id);
      ...
      function parseComicInfo(html) {
        try {
          ...
          let cover = YKMH_MOBILE_DEFAULT_COVER;
          try {
          ...
          return {
            title: title || "未知标题",
            cover: cover ? toYkmhMobileUrl(cover) : YKMH_MOBILE_DEFAULT_COVER,
            author: author || "未知作者",
            status: status || "未知状态",
            tags: tags || [],
                ...
                while (
                  ...
                  try {
                    if (chapterMatch && chapterMatch[1] && chapterMatch[2]) {
                      let chapterUrl = toYkmhMobileUrl(chapterMatch[1]);
            ...
            while ((match = allChapterPattern.exec(html)) !== null) {
              try {
                if (match && match[1] && match[2]) {
                  let chapterUrl = toYkmhMobileUrl(match[1]);
      ...
      try {
        ...
        if (!comicInfo.cover)
          comicInfo.cover = YKMH_MOBILE_DEFAULT_COVER;
    ...
    loadEp: async (comicId, epId) => {
      ...
      let chapterUrl = toYkmhMobileUrl(
        replaceWebSourceBaseUrl(epId, YKMH_DESKTOP_BASE_URL, YKMH_MOBILE_BASE_URL),
      );
      ...
      function parseChapterImages(html) {
        ...
        if (scriptMatch) {
          try {
            ...
            images = imageList.map((img) => toYkmhMobileUrl(img));
          } catch (e) {}
        }
        if (images.length === 0) {
