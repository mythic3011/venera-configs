...
const refreshKomgaReferenceDataFeature = createSelfHostedReferenceCacheFeature({
  ...
  hasToken: (source) => Boolean(source.authToken),
  loadPayload: async (source) => {
    const [libraries, tags, languages, collections, genres] = await Promise.all([
      source.getJson(KOMGA_ROUTES.librariesPath()),
      source.getJson(KOMGA_ROUTES.seriesTagsPath()),
      source.getJson(KOMGA_ROUTES.languagesPath()),
      source.getJson(KOMGA_ROUTES.collectionsPath(), {
        unpaged: true,
        sort: ["name,asc"],
      }),
      source.getJson(KOMGA_ROUTES.genresPath()),
    ]);
    return { libraries, tags, languages, collections, genres };
  },
  savePayload: (source, payload) => {
    const libraryList = Array.isArray(payload?.libraries)
    ...
    const collectionList = Array.isArray(collectionPage?.content)
      ...
      : Array.isArray(payload?.collections)
        ? payload.collections
        : [];
    source.saveData("komga_libraries", libraryList);
    source.saveData("komga_tags", Array.isArray(payload?.tags) ? payload.tags : []);
    source.saveData(
      "komga_genres",
    ...
    source.saveData("komga_collections", collectionList);
  },
  shouldRethrow: (error) => String(error) === "Login expired",
});

const initKomgaFeature = createSafeInitFeature((source, force) =>
  refreshKomgaReferenceDataFeature(source, force),
);
const KOMGA_ROUTES = createKomgaRouteHelpers();
...
class Komga extends ComicSource {
  ...
  url = resolvePluginUpdateUrl("komga.js");
  ...
  get headers() {
    return withBasic({ Accept: "application/json" }, this.authToken);
  }
  ...
  account = {
    login: async (account, pwd) => {
      ...
      const res = await Network.get(this.buildUrl(KOMGA_ROUTES.currentUserPath()), {
        ...withBasic({ Accept: "application/json" }, token),
      });
      if (res.status === 401) {
        throw "账号或密码错误";
      }
  ...
  explore = [
    {
      ...
      load: async () => {
        ...
        const feeds = {};
        const latest = await this.fetchSeriesList(KOMGA_ROUTES.latestSeriesPath(), {
          size: 12,
          page: 0,
        });
        if (latest.comics.length) feeds["最新上架"] = latest.comics;
        const updated = await this.fetchSeriesList(KOMGA_ROUTES.updatedSeriesPath(), {
          size: 12,
          page: 0,
        });
        ...
        if (Array.isArray(libraries)) {
          for (const library of libraries.slice(0, 4)) {
            const list = await this.fetchSeriesList(KOMGA_ROUTES.seriesPath(), {
              page: 0,
              size: 12,
  ...
  category = {
    title: "Komga",
    parts: [
      createStaticCategoryPart("常用", "all", "all", null),
      ...
      createStoredCategoryPart({
        ...
        getParam: (collection) => collection?.id,
      }),
      createStoredCategoryPart({
        partName: "标签",
  ...
  categoryComics = {
    load: async (category, param, options, page) => {
      ...
      if (category === "all") {
        ...
        // }
        const list = await this.fetchSeriesList(KOMGA_ROUTES.seriesPath(), query);
        return {
          comics: list.comics,
          maxPage: Math.max(1, list.totalPages),
        };
      ...
      if (category === "collection" && param) {
        const list = await this.fetchSeriesList(
          KOMGA_ROUTES.collectionSeriesPath(param),
          query,
        );
        return {
          comics: list.comics,
  ...
  comic = {
    loadInfo: async (id) => {
      ...
      const [series, booksPage] = await Promise.all([
        this.getJson(KOMGA_ROUTES.seriesDetailsPath(id)),
        this.getJson(KOMGA_ROUTES.seriesBooksPath(id), {
          unpaged: true,
      ...
      const info = new ComicDetails({
        ...
        cover: this.buildUrl(KOMGA_ROUTES.seriesThumbnailPath(id)),
        description,
        tags: tagSections,
        chapters,
        updateTime: this.formatDate(series?.lastModified),
        uploadTime: this.formatDate(series?.created),
        url: series?.url || this.buildUrl(KOMGA_ROUTES.seriesWebPath(id)),
      });
      return info;
    },
    loadEp: async (comicId, epId) => {
      ...
      const pages = await this.getJson(KOMGA_ROUTES.bookPagesPath(bookId));
      ...
      const images = list
        ...
        .map((page) => {
          ...
          return this.buildUrl(
            KOMGA_ROUTES.bookPageImagePath(bookId, number),
  ...
  async refreshReferenceData(force) {
    await refreshKomgaReferenceDataFeature(this, force);
  }
  ...
  parseBook(book) {
    ...
    return new Comic({
      ...
      cover: this.buildUrl(KOMGA_ROUTES.bookThumbnailPath(book.id)),
  ...
  async loadBookDetails(bookId) {
    const book = await this.getJson(KOMGA_ROUTES.bookDetailsPath(bookId));
    ...
    return new ComicDetails({
      ...
      cover: this.buildUrl(KOMGA_ROUTES.bookThumbnailPath(bookId)),
      description,
      tags: tagSections,
      chapters,
      updateTime: this.formatDate(book.lastModified),
      uploadTime: this.formatDate(book.created),
      url: book.url || this.buildUrl(KOMGA_ROUTES.bookWebPath(bookId)),
    });
  }
  ...
  parseSeries(series) {
    ...
    return new Comic({
      ...
      cover: this.buildUrl(KOMGA_ROUTES.seriesThumbnailPath(series.id)),
      tags: this.uniqueArray(tags).slice(0, 12),
      description,
    });
  }
