...
function parseHitomiGgPrefix(script) {
  const body = String(script || "");
  const direct = body.match(/\bgg\.b\s*=\s*["']([^"']+)["']/);
  if (direct && direct[1]) {
    return direct[1];
  }
  const objectStyle = body.match(/\bb\s*:\s*["']([^"']+)["']/);
  if (objectStyle && objectStyle[1]) {
    return objectStyle[1];
  }
  return "";
}

function createSafeGgRuntime(script) {
  const prefix = parseHitomiGgPrefix(script);
  return {
    b: prefix,
    s(hash) {
      return String(hash || "").replace(/^.*(..)(.)$/, "$2/$1/");
    },
    m(seed) {
      if (typeof seed !== "number" || !isFinite(seed)) {
        return 0;
      }
      // Keep subdomain rotation deterministic without dynamic script execution.
...
async function get_image_srcs(files) {
  ...
  const gg = createSafeGgRuntime(resp.body);
...
class Hitomi extends ComicSource {
  ...
  // update url
  url = resolvePluginUpdateUrl("hitomi.js");
