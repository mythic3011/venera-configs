...
test("kavita artifact boots under runtime shim", () => {
  const repoRoot = path.resolve(__dirname, "../..");
  const manifestPath = path.join(repoRoot, ".generated", "build-manifest.json");
  const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
  const plugin = manifest.plugins.find((entry) => entry.id === "kavita");
  assert.ok(plugin, "kavita is missing from build manifest");
  const source = fs.readFileSync(path.join(repoRoot, plugin.outputPath), "utf8");
  const context = createVeneraHostShim();
  vm.runInContext(`${source}\nthis.__Ctor__=Kavita;`, context, { filename: plugin.outputPath });
  const instance = new context.__Ctor__();
  assert.equal(instance.key, "kavita");
