...
function buildMhLikeSearchUrl(config) {
  const options = config || {};
  const searchPath = normalizeMhLikePath(options.searchPath, MH_LIKE_ENDPOINT_PATHS.SEARCH);
  const keyword = encodeURIComponent(String(options.keyword == null ? "" : options.keyword));
  return buildMhLikeRelativeUrl(
    options.baseUrl,
...
function buildMhLikeChapterUrl(baseUrl, chapterId) {
  return `${String(baseUrl || "").replace(/\/+$/, "")}/${normalizeMhLikeId(chapterId)}`;
}
