...
function buildCopyLikeRankingUrl(config) {
  ...
  return buildCopyLikeUrlWithQuery(options.apiUrl, COPY_LIKE_ENDPOINT_PATHS.RANKS, [
    ...
    [
      "offset",
      options.offset == null
        ? buildOffsetByPage(options.page, options.limit == null ? 30 : options.limit)
        : options.offset,
    ],
...
function buildCopyLikeSearchUrl(config) {
  ...
  const keyword = options.keyword == null ? "" : encodeURIComponent(String(options.keyword));
  return buildCopyLikeUrlWithQuery(
    options.apiUrl,
...
function buildCopyLikeDetailTagMap(comicData, options) {
  ...
  const updateValue = comicData && comicData.datetime_updated ? comicData.datetime_updated : "";
  const statusValue =
